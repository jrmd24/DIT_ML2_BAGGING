...
class CustomRandomForest:
    def __init__(
        ...
        for _ in range(self.num_of_trees):
            ...
            self.trees.append(
                CustomDecisionTree(
                    self.target, selected_features, self.start_depth, self.max_depth
                )
            )
    ...
    def predict_one(self, row):
        ...
        if len(most_common) == 1:
            ...
            most_common_value = (
                self.default_class
                if self.default_class in top_classes
                else random.choice(top_classes)
            )  # tie resolved by favoring default majority class
...
###################################################"
    ...
    rf_acc_std_dev = np.std(rf_acc_list)
    ...
    plt.axhline(
        y=rf_acc_std_dev,
        color="green",
        label=f"RF Accuracy std deviation = {rf_acc_std_dev:.2f}",
    )
    plt.title("RF VS Simple Decision tree accuracy standard deviation")
    plt.xlabel("Number of Trees")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.savefig(
        os.path.join(
            CM_OUTPUT_DIR, f"RF VS Simple Decision tree accuracy standard deviation.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )
